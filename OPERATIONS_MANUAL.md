...
|---|---|---|---|---|
| M0 工程地基 | ✅ 已交付 | 2026-06-16 | `m0` | G0: test绿✅/ping真调✅/远程首commit✅；CI已触发(私有库无token暂无法轮询，本地等价检查全绿) |
| M1 单 Agent 基线 | ✅ 已交付 | 2026-06-16 | `m1` | G1全过: 18测试绿/5意图端到端真调/MCP独立可调/自建退款集pass@1=1.0(8/8) |
| M2 退款风控闭环 | ✅ 已交付 | 2026-06-16 | `m2` | G2全过: 幂等/分流/HITL中断+跨会话续单/故障降级/RAG版本化/自建32条pass@1=1.0; BFCL-style 0.8 |
| M3 评测与消融 | ✅ 已交付 | 2026-06-16 | `m3` | G3全过: make eval全指标+CI+pass^k(32任务pass^3=0.875); E1(L0→L1→RAG)/E3消融; 错误分类自动; 回归CI实测可拦 |
| M4 收尾交付 | ✅ 已交付 | 2026-06-16 | `m4` | G4: 6条闭环全取证; FastAPI+Web(对话+trace可视化); make demo; E6 Pareto; README/ARCHITECTURE定稿 |
...
### 2.1 验收取证(实测,2026-06-16)

| # | 维度 | 客观证据(可复现) |
|---|---|---|
| ① | 真实业务规则 | `BUSINESS_RULES.md`(版本 2026.06)+ `policy/store.py` 代码强制阈值;RAG 命中带 version |
| ② | 端到端可运行 | `make demo`(HITL+跨会话续单)/ `make serve` Web UI;FastAPI `/chat`→HITL→`/confirm`→开单实测通过 |
| ③ | 可观测 trace | 每次 tool/interrupt/guardrail decision/失败落 JSON;Web 右栏实时可视化 |
| ④ | 可恢复 HITL | LangGraph `interrupt()` + SqliteSaver;demo 实测 Day1 暂停→Day2 新会话 resume 开单 |
| ⑤ | 可评测 benchmark | `make eval` → 32 任务 pass^1=0.948/pass^3=0.875+CI+合规+错误分类;`eval.bfcl`/`ablations`/`pareto` |
| ⑥ | 可回归 CI | GitHub Actions: lint+pytest(38)+`eval.regression`(model-free 安全门);实测注入回归→红、还原→绿 |
...
- [x] §2 六条验收闭环全部有客观证据(见 §2.1)
- [x] 全套交付物齐备:`README` / `ARCHITECTURE` / `BUSINESS_RULES` / `tools/schema.py` / `graph/` / `eval/` / `web/` / `reports/ablation_report.md` / `reports/error_taxonomy.md` / Demo(`reports/demo_transcript.md`)
- [x] `make demo` 端到端跑通(真实调用 DeepSeek v4)
- [x] `make eval` 跑出全套指标 + 置信区间 + pass^k(32 任务 pass^3=0.875)
- [x] eval 回归门(`eval.regression`)进 CI,实测能在回归时变红
- [x] 代码推送到 GitHub,里程碑 tag `m0`–`m4` 齐全
- [x] 进度看板(§0)全部 ✅
